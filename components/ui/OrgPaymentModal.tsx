...
interface OrgPaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  amount: number;
...
export function OrgPaymentModal({ 
  isOpen, 
  onClose, 
  amount,
  paymentMethod,
  ...
  const dummyQR = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=OrgPayment_${amount}_${paymentMethod}`;
  ...
  const getMethodLabel = () => {
    switch (paymentMethod) {
      ...
      default: return 'Dompet Organisasi';
    }
  };
      ...
      <DialogContent className="sm:max-w-md bg-forest-card border-forest-border">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-forest-beige flex items-center gap-2">
            {isWallet ? (
              <><Wallet className="w-5 h-5 text-emerald-400" /> Pembayaran via Dompet</>
            ) : (
          ...
          <div className="space-y-6 pt-4">
            <div className="bg-[#181A15] p-4 rounded-xl border border-forest-border space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-forest-muted">Jumlah Pembayaran</span>
                <span className="text-forest-beige font-bold">Rp {amount.toLocaleString('id-ID')}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-forest-muted">Saldo Dompet Organisasi</span>
              ...
              <div className="border-t border-forest-border/50 pt-3 flex justify-between">
                <span className="text-forest-muted">Sisa Saldo</span>
                <span className={`font-bold ${isInsufficient ? 'text-red-400' : 'text-emerald-400'}`}>
                  {isInsufficient ? '-' : ''} Rp {Math.abs(balance - amount).toLocaleString('id-ID')}
                </span>
              </div>
            </div>
            ...
            {isInsufficient ? (
              <div className="space-y-4">
                <div className="flex items-start gap-3 p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 text-sm">
                  ...
                  <p>Saldo dompet organisasi tidak mencukupi. Silakan top up dompet terlebih dahulu melalui halaman Wallet.</p>
                </div>
                <Button 
                  onClick={onClose} 
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl text-center">
                ...
                <p className="text-2xl font-bold text-amber-400 mt-1">Rp {amount.toLocaleString('id-ID')}</p>
              </div>
