...
import { MessageSquare, Plus, Search, Filter, Clock, CheckCircle2, ChevronRight, Inbox } from "lucide-react";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-forest-beige tracking-tight flex items-center gap-2">
            <MessageSquare className="w-8 h-8 text-forest-accent" />
            My Support Tickets
          </h1>
          <p className="text-forest-muted mt-1">Track the status of your support requests.</p>
        </div>
        ...
        <CardContent className="p-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-[#181A15] rounded-xl border border-dashed border-forest-border">
              ...
              <p className="text-forest-muted mb-4">You haven't submitted any support requests yet.</p>
              <Link href="/help/submit">
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {filteredTickets.map((ticket) => (
                    <tr key={ticket.id} className="border-b border-forest-border hover:bg-[#181A15]/50 transition-colors group cursor-pointer" onClick={() => window.location.href = `/help/tickets/${ticket.id}`}>
